        ...
        for instruction in instructions:
            ...
            if not dimension in dimensions: # New dimension:
                ...
                if len(list(value)) == len(values[dimension][0]):
                    ...
                    raise ValueError('Sweeping values of {:s} and {:s} are inconsistent!'.format(sweep.name,sweeps[dimension][0].name)) 
...
#                     real_name = temp_name.split(str_offs_link)[0]
    ...
    def execute_sweep(self,
        ...
        self.register_sweep_info(meas, instructions, readouts)             # Register sweep information
        register_pre_and_post_process(meas,pre_process,post_process)       # Register pre- and post- processes
